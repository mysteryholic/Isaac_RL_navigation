...
class IsaacEnvROS2(Node):
    def __init__(self, size, angle_bins: int, z_bins: int):
        ...
        # self.voxel_data = torch.from_numpy(np.zeros((3,z_bins,angle_bins), dtype=np.float32))
        self.voxel_data=torch.from_numpy(np.zeros((30,2), dtype=np.float32))
        self.goal_state = torch.from_numpy(self.goal - self.pose[:2])
        self.next_state = (self.voxel_data, self.goal_state)
        self.next_state_add= np.array([self.goal[0]-self.pose[0],self.goal[1]-self.pose[1],self.goal[0], self.goal[1], self.pose[2],0.0001],dtype=np.float32)
    ...
    def lidar_cb(self, msg):
        ...
        # self.voxel_data = torch.from_numpy(np.array(msg.data, dtype=np.float32).reshape(3,self.z_bins,self.angle_bins))
        self.voxel_data=torch.from_numpy(np.array(msg.data, dtype=np.float32).reshape(30,2))
    ...
    def reset(self):
        ...
        # Convert goal to local robot coordinates
        goal_vector_global = self.goal - self.pose[:2]
        theta = self.pose[2]  # Robot's yaw angle
        
        # Apply rotation matrix to transform to local coordinates
        goal_vector_local_x = goal_vector_global[0] * np.cos(theta) + goal_vector_global[1] * np.sin(theta)
        goal_vector_local_y = -goal_vector_global[0] * np.sin(theta) + goal_vector_global[1] * np.cos(theta)
        
        self.next_state_add = np.array([goal_vector_local_x, goal_vector_local_y,self.goal[0], self.goal[1], self.pose[2],0.0001],dtype=np.float32)
    ...
    def step(self, action, time_steps, max_episode_steps):
        ...
        self.next_state_add = np.array([goal_vector_local_x, goal_vector_local_y,self.goal[0], self.goal[1], self.pose[2],action[0]],dtype=np.float32)
        ...
        return self.next_state, self.next_state_add, reward, self.done,self.goal_check, self.cylinder_coords
...
if __name__ == '__main__':
    ...
    size = 5.0
    angle_bins, z_bins,division,num_selectpoints = 180, 10,10,3
    lidar_preprocessing = LidarPreprocessing(angle_bins, z_bins,division,num_selectpoints)
    ...
    input_shape = (angle_bins, z_bins,division,num_selectpoints)
    print(f"env_state_add shape:{len(env.next_state_add)}")
    print(f"env_state:{env.next_state_add}")
    agent = TD3(input_shape, len(env.next_state_add), action_space, args)
    ...
    destination=np.array([env.size, env.size], dtype=np.float32)
    destination[1]=env.goal[1]
    add_x=1
    add_y=0
        ...
        while rclpy.ok():
            for i_episode in itertools.count(1):
                ...
                while not done:
                    ...
                    yaw=env.pose[2]
                    angle=math.atan2(destination[1]-env.pose[1], destination[0]-env.pose[0])-yaw
                    
                    if total_numsteps<20:
                        action=np.random.uniform(-math.pi/8, math.pi/8, size=action_space)
                        actor_loss=0
                    else:
                        action,actor_loss=agent.select_action(state, state_add)
                        print(f"origin action:{action}")
                        # angle=(angle/2).clip(-math.pi/8, math.pi/8)
                        # action+=angle
                        # action.clip(-math.pi/2, math.pi/2)

                        action += np.random.normal(-expl_noise, expl_noise, size=action_space)
                        action = action.clip(-math.pi/4, math.pi/4)

                    if len(memory) > args.batch_size:
                        av_critic_loss, av_Q, max_Q,coordinate = agent.update_parameters(memory, args,expl_noise)
                    ...
                    writer.add_scalar('actor_loss', actor_loss, total_numsteps)
                        
                    next_state, next_state_add, reward, done,goal_check,cylinder = env.step(action, episode_steps, max_episode_steps)
                    episode_steps += 1
                    total_numsteps += 1
                    episode_reward += reward

                    mask = 1 if episode_steps == max_episode_steps else float(not done)
                    # state = np.clip(state, 0.0, 5.1)
                    state_add = np.clip(state_add,-math.pi/2, math.pi/2)
                    memory.push(state, state_add, action, reward, next_state, next_state_add, mask,td_error)
                    state = next_state
                ...
                if i_episode % 20 == 0 and args.eval is True:
                    ...
                    for i  in range(episodes):
                        ...
                        while not done:
                            action,actor_loss2= agent.select_action(state, state_add)
                            next_state, next_state_add, reward, done,goal_check,td_error = env.step(action, eval_steps, max_episode_steps)
                            episode_reward += reward
                        ...
                        writer.add_scalar('evaluation actor loss',actor_loss2,eval_count)
                    avg_reward /= episodes
