...
class TD3(object):
    ...
    def update_parameters(self, memory, args,angle):
        max_Q=float('-inf')
        av_Q=0
        av_loss=0
        ...
        max_reward=torch.max(abs(max_reward),abs(min_reward))
        reward_batch = (reward_batch-min_reward)/max_reward
        ...
        # noise=PotentialFieldForce(goal, pose, voxel_data).compute_action()
        # noise = torch.tensor(noise).to(self.device)
        next_action = (next_action + angle).clamp(-math.pi/4,math.pi/4)
        ...
        if self.count % self.policy_freq == 0:
            ...
            print("Q1.shape",self.critic.Q1(state_batch, state_batch_add, self.actor(state_batch, state_batch_add)).shape)
            actor_grad = weights*self.critic.Q1(state_batch, state_batch_add, self.actor(state_batch, state_batch_add))
    ...
    def update_cbam(self, state,state_add,action, target_Q):
        ...
        current_Q1=self.critic.Q1(current_cbam, state_add, action)
        loss=-current_Q1.mean()
